...
class XAI_Backend:
    ...
    def _build_and_train_model(self):
        ...
        for _ in range(self.cfg['model']['epochs']):
            optimizer.zero_grad(); criterion(self.model(X_t), y_t).backward(); optimizer.step()
        self.model.eval()

    def _init_explainers(self):
        background = torch.tensor(self.X_train.values[:self.cfg['xai']['shap_background_samples']], dtype=torch.float32)
        self.shap_explainer = shap.DeepExplainer(self.model, background)
        d = dice_ml.Data(dataframe=self.df_scaled, continuous_features=self.numerical, outcome_name=self.target_col)
    ...
    def get_explanation(self, instance_idx):
        ...
        with torch.no_grad(): prob = self.model(patient_tensor).item()

        # SHAP
        shap_values = self.shap_explainer.shap_values(patient_tensor)
        if isinstance(shap_values, list): shap_values = shap_values[0]
        shap_list = sorted(zip(self.X_test.columns, shap_values.flatten()), key=lambda x: abs(x[1]), reverse=True)
        
        # Actionable Binding
        actionable_pool = self.cfg['data']['actionable_features']
        dynamic_actionable = [f for f, imp in shap_list if f in actionable_pool and abs(imp) > self.cfg['xai']['shap_relevance_threshold']]
        if not dynamic_actionable: dynamic_actionable = actionable_pool

        # Counterfactual Logic
        status, reduced_prob, real_target = "Error", prob, real_orig.copy()
        try:
            dice_exp = self.dice_explainer.generate_counterfactuals(patient_row, total_CFs=1, desired_class=0, features_to_vary=dynamic_actionable)
            if dice_exp.cf_examples_list[0].final_cfs_df is not None:
                cf_df = dice_exp.cf_examples_list[0].final_cfs_df
                real_target = get_real_world_values(cf_df)
                status = "Path Found"
                with torch.no_grad():
                    reduced_prob = self.model(torch.tensor(cf_df.drop(self.target_col, axis=1).values, dtype=torch.float32)).item()
        except Exception:
            status = "Heuristic"
            ...
            for f in dynamic_actionable:
                if f in self.numerical:
                    ...
                    if patient_row.iloc[0][f] > val: best_case[f] = val
            real_target = get_real_world_values(best_case)
            with torch.no_grad(): reduced_prob = self.model(torch.tensor(best_case.values, dtype=torch.float32)).item()

        # --- SELECT ONLY CHANGED FEATURES ---
        # We only want features where the difference is significant (> 0.1 units)
        changed_features = [f for f in dynamic_actionable if abs(real_orig[f] - real_target[f]) > 0.1]
        ...
        diff_str = " | ".join([f"{f}: {real_orig[f]:.1f}->{real_target[f]:.1f}" for f in changed_features])
