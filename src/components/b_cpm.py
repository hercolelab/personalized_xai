...
class ContextualPreferenceModel:
    ...
    def initialize(self, role: str):
        ...
        if role in self.personas:
            ...
            print(
                f" [CPM] Warning: Role '{role}' not found in config. Using balanced defaults."
            )
    ...
    def update(self, dim_name: str, direction: int):
        ...
        self.vector[idx] = np.clip(
            self.vector[idx] + (self.alpha * direction), self.bounds[0], self.bounds[1]
        )
    ...
    def get_state(self):
        ...
        return {
            name: round(float(val), 2) for name, val in zip(self.dim_map, self.vector)
        }
