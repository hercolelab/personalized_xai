...
def clean_dataset(dataset_name):
    ...
    if dataset_name == "diabetes":
        ...
        columns_to_check = ['Glucose', 'BloodPressure', 'SkinThickness', 'BMI', 'Insulin']
        ...
        print(f"Original rows: {len(df)}, Cleaned rows: {len(df_cleaned)}, Removed: {len(df) - len(df_cleaned)}")
        
    else:
        raise ValueError(f"Dataset '{dataset_name}' is not supported. Currently only 'diabetes' is supported.")


def main():
    parser = argparse.ArgumentParser(description='Load and clean datasets')
    parser.add_argument('--dataset', type=str, required=True, help='Name of the dataset to load and clean')
